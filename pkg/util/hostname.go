...
func GetHostname() (string, error) {
	...
	if runtime.GOOS != "windows" {
		...
		if err == nil && !config.Datadog.GetBool("hostname_fqdn") && fqdn != "" && hostName == h && h != fqdn {
			log.Warnf("The agent resolved your hostname as '%s', and will be reported this way maintain compatibility with version 5. To enable reporting as %s, please enable the `hostname_fqdn` flag in the configuration. For more information: https://dtdg.co/flag-hostname-fqdn", h, fqdn)
		}
	}
